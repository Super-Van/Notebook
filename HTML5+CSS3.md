...
web标准提出的最佳体验方案是结构、样式、行为相分离，即html代码写进html文件、css代码写进css文件、js代码写进js文件。
...
见识了rem，再来见识一对新单位vw、vh。它们是面向未来的趋势，B站、小米等已经大规模使用。

vw（vh）与rem、百分比类似，都是比例型单位。rem的参照物是html字号；百分比的参照物是父元素；vw-viewport width的参照物是视口即屏幕宽度；vh-viewport height的参照物是视口即屏幕高度。显然它们实现了宽高的一齐缩放。

来看一下算法。给定宽度为$W$px、高度为$H$px的视口，某元素宽度设为$x$vw，高度设为$y$vw，则将其换算为px：
$$
xvw=x\frac{W}{100}px,yvh=y\frac{H}{100}px
$$
如对于375px、667px宽的屏幕，10vw=37.5px，2vh=13.34px。

开发时第一版样式往往是根据设计稿写起来的，故从像素换算到vw（vh）更为常见，那么根据上式不难得出：
$$
mpx=\frac{100m}{W}vw=\frac{m}{W/100}vw,npx=\frac{100n}{H}vh
$$
...
开发中vh很少使用，因为它们都是用于缩放的比例，只不过参照物不同，一个是屏幕宽，一个是屏幕高，我们用一种参照物就可，又因为在移动端宽度是大头，故我们选屏幕宽。从实践来看，我们本就从不纵向伸缩浏览器窗口。
