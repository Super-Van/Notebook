...
D:\programdata\MySQL\MySQL Server 8.0\my.ini： 配置信息
```
...
诞生于1974年的一篇论文，这几十年的变动很小。

虽然有统一的标准如SQL-99，但各数据库提供商对标准的具体实现不一致。
...
这是个很要紧的东西，实际开发中字段类型设置得不好，会带来错误或缺陷，如金额的精度不够、空间浪费、查询性能低等。
...
类型如何选取看各单位的要求，也可参考阿里巴巴Java开发手册等。
...
| INT或INTEGER | 4      | $[-2^{31},2^{31}-1]$  | $[0,2^{32}-1]$  |
...
M：显示宽度，须配合以ZEROFILL。如`INT(5)`，上限是11，这里给的5，作用是当分量位数小于5，高位用0补齐。当位数大于5，实际保存的数值并没有被截断。8.0.17开始不推荐使用此属性。

```sql
sno INT(5) ZEROFILL -- 自动还补上一个UNSIGNED，因为用零填充，就只能非负
```
...
浮点数与定点数支持小数，取值范围也比整数类型的大，但不够精确如极大整数的低位。
...
参考IEEE754规定的浮点数存储格式-固定位数的符号、尾数、阶码。
...
它就不像显示宽度，而是规定了存储值的范围，那么分量溢出的话，要分情况讨论：
...
存在精度损失（计算误差）问题，本质上是二进制计算机制造成的。由此引出定点数类型DECIMAL。
...
比较浮点数与定点数，同等容量条件下，前者范围更大，但不精确，后者相反。
...
YEAR的YY格式已不推荐使用，另外YYYY格式的YEAR没必要写成`YEAR(4)`，也体现了显示宽度不推荐。
...
相较于DATETIME，TIMESTAMP更适应时区变化、计算、排序相关的场景，因为底层TIMESTAMP存的是一个毫秒数，相应数据插入时会根据当前时区将其转为格林威治时间，再转为一个毫秒数，后续查询时仍根据当前时区格式化。
...
- 对CHAR，长度即字符数，那么一给定M，字符数就确定下来，但字节数不确定，因不确定会不会有中文字符。
- 对VARCHAR，长度指字节数，那么最坏情况下最多填$65535/3=21845$（UTF8条件下）个汉字，这21845也就是MySQL规定的字符数上限，即M的最大值。

空间上前者更浪费，时间上前者效率更高，故前者更适合存储不大、对速度要求高、定长的场景，后者相反。
...
BLOB系列适用于二进制大对象-图片、音视频等，但在实际开发中，一般这些多媒体资源不会存在数据库里，而存到服务器的硬盘中，数据库里保存的是它们的存储路径。
...
```sql
js JSON
VALUES ('{"name": "van", "age": 18, "address": {"provice": "hubei", "city": "huanggang"}}')
...
实际开发中要考量用不用外键。当不用时，想保证完整性，要么靠程序员的自觉，要么靠Java等应用程序逻辑。用的话，一旦数据量特别大、访问量也大，就会带来巨大的检查开销，如插入记录时的外键分量检查，影响插入效率。故建议开发中不要使用外键，而在应用层面实现引用完整性。
...
系统变量由系统定义，属服务器层面。启动MySQL服务，生成一个MySQL服务实例，就会为对应内存空间中的系统变量赋值，它们定义了当前实例的属性，如编码方式。它们要么来自编译MySQL软件时的参数，要么来自my.ini里的参数，我们只需关注常用变量。

系统变量又分为全局系统变量（全局变量）与会话系统变量（local变量）。

用户自定义变量又分为会话用户变量与局部变量，前者对当前会话有效，后者仅在BEGIN、AND语句块内有效。
...
代码举例参考SQL笔记文件trigger.sql，可交叉阅读[Oracle](oracle.md)笔记触发器一章。

优点：

- 保证数据完整性。如事务一致性-进货时金额的核算。
- 为操作做日志、为数据做备份。
- 保证数据数据合法性。如防止库管误输入价格。

缺点：

- 可读性差。尤当触发器数目非常多，有时报异常自己都一时弄不清。
- 表结构的变更可能引起触发器原有逻辑不合理，导致本来合理的操作报错。

外键约束造成的主表对副表的外键列操作不会引起触发器的执行，即触发器仅受本表内的操作驱动。

## 窗口函数

无论从执行效率、占用空间还是写法上，窗口函数都比普通的SQL语句要好很多。

机制与聚集函数相似，但后者是分组后必须汇总（聚集），前者可实现分组而不聚集然后由窗口函数将单行操作结果或聚集操作结果分给各组内记录，反正总记录数不被压缩。
...
接着就是安装那几个文件，按上面的顺序，对低版本centOS中途或许要执行`yum remove mysql-libs`，及强制安装[openssl-libs-1.0.2k-19.el7.x86_64.rpm](https://centos.pkgs.org/7/centos-x86_64/openssl-libs-1.0.2k-19.el7.x86_64.rpm.html)。
...
配置文件位于`/user/share/mysql-8.0`及`/etc/my.cnf`。

能查出有四个内置数据库：

- mysql：核心，存储了用户信息、存储过程的定义、日志、时区信息、帮助信息等。
- information_schema：存放DBS维护的其他库的信息，包括所含表、视图、触发器、索引等，这些信息又叫（用户数据的）元数据。
- performance_schema：存放DBS运行过程中的状态信息，用于监控各性能（空间上、时间上）指标。
- sys：主要用视图融合上两个库。

InnoDB、MyISAM之类的存储引擎负责将逻辑上的二维表存储到物理上的磁盘上，我们以他俩为例说明是怎么存放的。

对于InnoDB，比如创建了数据库atguigudb，再创建表student，那么在`/var/lib/mysql/atguigudb`里，就有student.ibd，此文件存放着表结构、表数据、索引等信息，验证如下：
...
对于MyISAM，比如又造了个course表，且指定引擎为MyISAM，那么发现在atguigudb目录下多了三个文件course_365.sdi、course.MYD、course.MYI，依次存放元数据、表数据与索引（D-data，I-index）。
...
所以8.0不用管了，但5.7有必要在my.cnf中修改编码方式与排列规则，最好在安装后马上改，再启动服务，否则改完重启服务，而且已建的库表不受影响。
...
权限表是MySQL进行权限控制的依据，上一节的收放权操作都落实为表里的记录。权限表位于系统库mysql中，包括user、db、table_priv、column_priv、proc_priv等，服务已启动，就将表里的信息读入内存。
...
同理执行`DESC tables_priv;`发现多了个Table_name字段，与Host、Db、User共同构成注解，体现针对某库某表的权限。
...
1. 连接核实就是从连接中取出用户名、主机地址、密码，去比对user表内host、user、authentication_string三个字段。

2. 请求核实就是沿着权限表从上级到下级（user到proc_priv）检查权限，只要有一张表匹配到Y值就说明有执行权限，否则继续往下级检查。

### 角色

角色是权限的集合，将角色赋予用户，它就拥有特定的一组权利。用户一多，往往存在若干用户拥有相同一组权利，此时就很适合被赋予角色。
...
SELECT CURRENT_ROLE();
-- 激活
SET DEFAULT ROLE 'boss'@'%' TO 'van'@'%', 'emp'@'%';
...
强制角色是任何用户创建之初就被赋予的角色，对其无法进行REVOKE或DROP。典例是带登录权限的角色。
...
```ini
[mysqld]
mandatory_roles='login@%,其他'
```

要么在服务运行时设置：

```sql
SET PERSISTENT mandatory_roles='login@%,其他'; # OS重启后仍有效
SET GLOBAL mandatory_roles='login@%,其他'; # OS重启后失效
```
...
可设定针对特定版本的选项组，如`[mysqld-5.7]`只能为5.7版本的服务所用。

当定义相同选项，后面的组覆盖前面的。当启动服务时给了重复选项，这又是覆盖。
...
- 统计表名命中率过低，归根结底是MySQL不够智能，对语句过于敏感，逻辑等价的语句写法上有一点不一样就被视为不同语句。
- 从某些系统函数、用户自定义变量、系统表得到的数据是不准被缓存的，如NOW函数两次得到一样的值就反常了。
...
解析器分析SQL语句，具体做词法分析与语法分析。前者对字符串与空格组成的序列，识别出当中每个元素指什么，如识别出关键字SELECT、表名employees、列名salary。后者检查语法有无问题。最终得到语法树，以下面这个语句为例，看树长什么样子。
...
同一条语句可能有不同的执行策略，比如是先筛选还是先连接，查询优化器旨在基于计算分析系统收集到的统计信息找出（它认为）最佳的执行计划。优化分为逻辑查询优化与物理查询优化，前者侧重于SQL逻辑的等价变换，如嵌套转连接，后者侧重于索引的使用与各表的连接顺序。

执行器负责执行计划，执行前会进行请求核实，不通过就直接返回权限错误，否则执行，要调用存储引擎的API，后者调用文件系统。
...
mysql> show profile for query 1; # 相同路径证明缓存关闭
+----------------------+----------+
| Status               | Duration |
+----------------------+----------+
| starting             | 0.000090 |
...
明显看出存储引擎与数据文件名的相对应，存储引擎以前叫表处理器（结果改了个故弄玄虚的名字），其功能是接收上层执行器传下来的指令，加载数据文件，对表中的数据进行读取或写入。

用`SHOW ENGINES;`查看诸引擎，结果展示了MySQL对它们的支持与否、它们支持事务与否、支持分布式与否、支持保存点与否。另发现默认是InnoDB（实际开发中较多使用），也可用如下语句查看默认引擎。

```sql
SHOW VARIABLES LIKE '%storage_engine';
SELECT @@default_strage_engin;
```

临时或永久地改用其他存储引擎：

```sql
SET default_strage_engin = MyISAM; -- 加SESSION、GLOBAL随意
```

```ini
default_strage_engin=MyISAM
# 别忘了重启服务
```
...
关于具体引擎，主要掌握InnoDB与MyISAM的区别，它们在实际应用中占比最大，各有各的优缺点，那么其他的了解即可。
...
- 长于处理大量短期事务，确保提交与回滚。
- 长于处理更新、删除操作。
- 数据量、并发量越大发挥出的性能越好，根本上是由于支持行锁（粒度更细），后面会详细说锁的知识。

一些缺点：

- 写效率稍差。
- 占用较多的空间保存数据与索引，因为它们存在一起。
- 因加载时不光加载索引，还加载数据，故占用更多内存、影响性能。

对比来看，MyISAM不支持事务、外键、行锁，及系统崩溃后无法安全恢复。

优点：

- 访问速度快，因为无视事务隔离性，以及长于处理查询、插入操作。
- 针对COUNT(*)等统计量查询效率更高，因为专门定义了常数实时记录。
- 加载时仅加载索引，索引数据分开存，故对内外层更友好。
...
索引是一种有序性快速查找数据结构，用于快速定位到记录，与之相对的是全表（记录逐一）扫描。本质上做到减少磁盘I/O的次数。

索引由存储引擎实现，故具体引擎实现的索引不尽相同，即意味着不尽相同的数据结构。引擎们定义各自的最大索引数与最大索引长度，所有引擎须满足每个表至少16个索引，总索引长度至少256个字节。
...
- 提升查询速度的同时降低更新表的速度，尤当频繁地增删改。

### 原理

以InnoDB的主键索引为例，具体演变不扯了，熟悉行格式、数据页、多级目录页，每页所带的页目录等概念，最后是N（B+树的层数）次磁盘IO，每次加载一个页的时候都对页目录中定义的主键分量组成的顺序表进行二分查找。

我们看形成的B+树，所有结点都是页，叶子结点是存放普通（原始数据）记录（0、2、3）的数据页，全体叶子结点按主键顺序构成双向链表（便于`ORDER BY`），叶子结点内部的原始记录也按主键顺序构成双向链表，非叶子结点是存放目录项记录（1）的数据页。页号无序不重复。

那么深度越低IO次数就越少，一般是设计4层，4层就已经能为一张表存储大量数据了，如InnoDB设定的页大小为16K，假定单页存100个记录，三层树就能存一百万条记录。一次加载一个磁盘页，这才有4层树对应4次IO的说法。
...
所谓聚簇，回想InnoDB所读写的ibd文件，是索引与数据是合一的，我们看B+树的结构便知，表面上数据展示的形式是二维表，底层就是存储成B+数，索引即数据。非聚簇就意味着索引结构中不包含数据。聚簇索引一定是B+树，而B+树也可能不作聚簇索引，关键就看含不含数据。

由于变存储组织数据边搭建B+树，即形成聚簇索引，故InnoDB自动创建聚簇索引，而非手动用语句创建。那么非聚簇索引就相反，因数据索引相分离，先有数据后有索引。

聚簇索引长于基于主键的精确或范围查找，缺点是不难想到的：

- 插入速度严重依赖插入顺序，按主键顺序插当然是最快的，故对InnoDB，提倡主键自增。
- 更新主键代价极大，故提倡主键分量不要更新。

其他要点：

- 就MySQL而言仅InnoDB支持聚簇索引。
- 主键就一个，表内数据也只一份，故聚簇索引只有一个。
- 若无主键，InnoDB先找一个<span id="unique">非空唯一</span>索引来代替，找不到则隐式定义一个主键，来作聚簇索引。
- 为满足结构里的顺序特性，尽量选一个有序的数值字段作主键。

#### 二级

二级索引又叫辅助索引，属非聚簇索引，针对的是基于非主键字段的查询。

观察B+树叶子结点中的记录，依该字段顺序构建双向链表，且除主键外无其他字段，因为索引与数据分离，这里的记录就不是一条完整的记录。由于不完整，如果想查其他字段或全字段就得查两次，第一次在二级索引上查出主键分量，第二次去聚簇索引查完整记录，此所谓二级之意，又叫回表。

那为什么叶子结点不干脆也存完整记录呢？索引一多，冗余空间太大了，我们牺牲时间换空间，B+树则体现了牺牲空间换时间。

二级索引的查询效率比不上聚簇索引，毕竟多查一次。

因二级索引也依赖主键，出于空间节省考虑，作主键的单字段长度要小，作主键的多字段数量要少。

#### 联合

联合索引在诸结点内部以多字段的顺序（如先按字段A排序，若A分量相等则按字段B排序）构建双向链表，每个数据页所存的记录均含多个字段（不包括主键）分量。属非聚簇索引，也依赖主键，适合基于多字段的查询。

#### InnoDB

理解InnoDB是怎么搭起B+树的。自然的想法是像前面那样自下而上演变，像从排到军的军队建制演变那样，其实是相反，它是从上到下搭建的。先有一个根结点，往里面填用户记录，等到填满了又来一条用户记录就建立第二层结点，并且把根结点里的用户记录复制进这一层结点，于是根结点里的记录就修改为目录项记录，等到根结点再次填满又来一条用户记录就建立第三层结点，同理将上层用户记录复制进这一层结点，于是二层结点里的记录也修改为目录项记录，根结点记录一并调整，如此迭代，随着用户记录越来越多，树越来越深。 

页内记录中的字段部分一定是[唯一](#unique)的，这也印证了BST的不重复性。因此我们看到二级索引也依赖主键。可以反证唯一性，一旦有增删改操作，索引就得维护（记录转移、页面增减等），即调整B+树结构，比如插入，若在当前结点发现两个记录的字段部分完全一样，就不确定去往哪一个子树了。

鉴于二叉树的结构特点，每个页至少要有两条记录。

#### MyISAM

MyISAM也采用B+树来做索引，只不过数据与索引分开，具体就是把叶子结点中记录里的非主键字段部分替换成记录的外存地址，如前所述，这个记录就位于MYD文件中，索引是MYI文件，故其下索引属非聚簇，又因查到某地址后去MYD文件查记录，也算回表，故属二级索引。

我们看MYI文件是自动生成的，说明默认生成一个索引，它满足唯一性但并不追求字段有序，那么构造树的过程中现有记录不会转移。但对自己创建的索引，构造树会考虑字段的顺序。

#### 其他结构

全表扫描效率最低，平均时间复杂度为$0(n)$。

查精确值，哈希表的平均时间复杂度为$O(1)$，但对大范围查询、索引字段的`ORDER BY`比不上B+树，也不适合高重复率字段等。InnoDB吸收哈希表的优点，针对高频查询字段定义了自适应哈希索引。

```sql
SHOW VARIABLES LIKE '%adaptive_hash_index%';
```

BST的时间平均复杂度为$O(log_2n)$，但按插入顺序构造的树可能极不平衡，甚至退化成链表，深度特别大。

AVL的最坏时间复杂度就$O(log_2n)$，然后可推广到M叉树（M>2），进一步降低深度。

B树是M叉平衡搜索树，结合索引来看内结点记录包括用户记录，全体叶子结点并不形成链表，便不同于B+树，后者也非搜索树。等大页空间条件下，显然B+树的页能存更多的目录项记录，故子树数更大，深度更低。

### 使用

补充一些概念：普通索引-无任何限制条件的索引（如不要求依赖的字段非空）、唯一索引-字段组的唯一性与唯一索引相辅相成、主键索引、单列索引、多列索引、全文索引-利用分词技术等算法依赖FULLTEXT字段的索引（逐渐被ES替代）、降序索引-8.0之后InnoDB才支持的按字段降序构造B+树、隐藏索引-不删除而控制索引优化查询与否（主键索引不能被隐藏）。
...
- DINSTINCT修饰的字段。
- 连接时，表不要超过3张，连接字段也须有索引。
- 考虑索引占空间，字段类型取容量小的比较好。
- 为字符串创建前缀索引，依选择度`COUNT(DISTINCT LEFT(字段，长度)) / COUNT(*)`趋近1选取最佳前缀长度。前缀索引不适合`ORDER BY`。
- 区分度（散列度）高-列的基数（不重复分量数）大的字段，一般选择度超过33%。同理联合索引中区分度高的应靠左声明-最左前缀原则。
...
那么不适合创建索引的情况就是前面列表的反面。补充一些：数据量小的表、经常更新的表、取值成无序性的字段（如UUID、身份证号）、冗余的索引（如已经有了针对`(sno, sname)`的联合索引，又来了个创建针对`sno`的单列索引）。
...
- 语句效率已达到瓶颈，做读写分离、分库分表。

从SQL语句及索引、数据表、DBS配置、硬件这几个维度调优，越往右成本越大且效果越弱。
...
可用前面提到的`SHOW PROFILE`命令查看查询慢的语句的执行详情，不过此命令将被弃用，代以查询information_schema库里的profiling表。

EXPLAIN（或DESCRIBE）帮我们查看优化器生成的执行计划。只看执行计划并不实际执行。
...
查询这些视图会使得服务器消耗大量资源取收集信息，故生产上不要频繁查询。
...
优化器是否使用索引主要考虑运行成本。与算法的复杂度类似，这个成本的核心指标不是时间，因为时间不够绝对，深受CPU等影响。另外考虑版本、数据量、SELECT子句的选择度等。

对WHERE子句中AND连接的多字段条件，最好有全字段索引：

```sql
SELECT * FROM student WHERE name = 'li' AND age = 30 AND gender = 'M';
ALTER TABLE student CREATE INDEX idx_name_age_gender(name, age, gender);
```
...
计算、函数、类型转换等导致索引失效，故慎用。想象函数对字段所有分量进行处理。

观察联合索引字段顺序，确保处范围（BETWEEN、比较运算符、IN、OR）条件中的字段排最后，否则索引失效。

```sql
ALTER TABLE student CREATE INDEX idx_name_age(name, age);
-- 优化器会自动调整，将age > 20挪到尾部
SELECT * FROM student WHERE age > 20 AND name = 'li'
```

条件含`!=`或`<>`时索引失效，同理可知含`IS NOT NULL`时索引失效，`IS NULL`则不会，由此推荐设计表时对全字段设非空约束，或者给默认值，一并缩减行格式中NULL值所占空间。

LIKE后面接以通配符`%`开头的值，索引会失效。

对OR，若前后存在列或列组对应的索引不完全”吻合“，则索引失效。像把本节首例中的AND都替换成OR，索引就失效。
...
库、表的编码方式要统一，否则在匹配是由编码方式的转换引起索引失效。

上述失效并不都是绝对的，要切实地看优化器的成本权衡，总之尽量避免索引失效（不被优化器采纳）。

覆盖索引：二级索引不用回表（随机IO优化为顺序IO），因为SELECT子句中的字段含于二级索引涉及的字段。

EXPLAIN结果中Extra分量可取Using index condition，表示产生索引条件下推（index condition pushdown, ICP），ICP意思是当使用到二级索引，先进行基于其他条件的筛选再回表，从而减少回表次数即减少随机IO。

```sql
ALTER TABLE student ADD idx_age_name(age, name);
-- 部分使用此索引查找age < 12的记录，且找到时一并判断name条件，最后回表，而不是找到age < 12的记录后先回表，再继续过滤
SELECT * FROM student WHERE age < 12 AND name LIKE '%ang';
-- 完全使用索引，就不产生ICP了，像聚簇索引、覆盖索引也不产生，因不回表
...
对于连接查询，连接字段类型须一致，否则产生隐式转换，索引失效。具体地，对左外连接或右外连接，一般说来被驱动表是固定的右表或左表，故应给被驱动表的连接字段添加索引，而对内连接，仅一方被添加索引连接字段所属的表会被优化器当作被驱动表，若两方都有或都没有索引，则优化器权衡运行成本，一般小结果集（过滤记录条数$\times$字段组总容量）表驱动大结果集表。

MySQL只支持一种表间关联方式，即嵌套循环（nested loop join），从数据库理论上看叫笛卡尔积。驱动表也叫主表，被驱动表也叫从表。对内连接，驱动表不固定为左表或右表。甚至对外连接，驱动表也不一定固定，因为有时两表皆不存在不满足连接条件的冗余记录，即相当于内连接了。

效率从低到高排：简单嵌套循环连接（simple nested-loop join）；块嵌套循环连接（block nested-loop join）-无索引，驱动表以块（若干条分量组）为单位加载到join buffer并以一块匹配被驱动表的每一条；索引嵌套循环连接（index nested-loop join）。

```sql
-- 查看join buffer大小，默认是16K
SHOW VARIABLES LIKE 'join_buffer%';
```

为让join buffer存放更多数据，不难想到应尽量增大其容量并让驱动表SELECT子句中字段组总容量尽量小。

8版本废弃了BNLJ，而引入hash join-为较小的结果集建立散列表，扫描大结果集表，取每行来探测散列表，找出匹配的行，仅适用于等值连接。

##### 嵌套

子查询会带来临时表的创建与销毁，有时带来极大的CPU、IO等资源的开销。临时表不能有索引，总查询性能受影响，尤其子查询涉及的数据量很大。故最好将嵌套查询转化为等价的连接查询。

##### 排序

MySQL支持两种排序-文件排序与索引排序，前者效率更低，占用CPU较多，甚至有时数据量大了产生临时IO。

有时WHERE子句与ORDER BY子句中的字段不同，可建立联合索引：

```sql
-- 联合索引中列的顺序应与WHERE、ORDER BY子句所涉及字段的顺序一致
ALTER TABLE student ADD INDEX idx__(home, age);
SELECT * FROM student WHERE home = 'Beijing' ORDER BY age;
-- 不过有时因筛选出来的数据量过大，第二列不被采用（key len缩短），加LIMIT可能解决
```

针对多字段排序，联合索引中列的顺序与ORDER BY字段中的字段顺序应一致，且诸字段升降序应统一。

```sql
SELECT * FROM student ORDER BY home DESC, age DESC;
```

有时只能用文件排序，那么考虑它如何优化。它底层的算发又分两种-双路排序和单路排序，一般的优化策略如下。

```sh
# 尽量增大此变量（单次加载的数据量）值，[1M, 8M]
...
+-------------------------+-----------+
3 rows in set, 1 warning (0.00 sec)
# 尽量增大此变量值-专门增大单路排序在内存中的排序空间，即专门减少单路排序的磁盘IO，同时避免使用SELECT *，即减少其他字段分量的占用 [1024, 8192]
...
GROUP BY子句也可依赖索引，因为本质上分组也是靠等值匹配展进行的。
...
IN和EXISTS怎么选，若子查询得到小结果集表，则用IN，在外查询的大结果集表上建立索引，否则用EXISTS，在子查询大结果集表上建立索引，毕竟数据规模越大索引越狠。

关于COUNT，当使用`COUNT(具体字段)`，最好建立相应二级索引，当使用`COUNT(*)`或`COUNT(1)`，则DBS会自动采用占空间更小（根据key len）的二级索引进行统计，没有二级索引就采用主键索引。

关于`SELECT *`，尽量避免，前面提过一些原因，补充一个明显的原因-做语法解析的时候会查数据字典将`*`转化成所有列名，费时。

针对全表扫描的语句，若结果集中含任意一条记录就能满足需求，则最好给语句加个`LIMIT 1`。
...
ORDER BY、GROUP BY、DISTINCT比较耗费CPU资源，故减少使用，WHERE子句过滤出的记录数控制在1000以内，否则陡然变慢。

#### 主键设计

对核心业务不推荐自增值：暴露业务量如用户量、底层性能较差、分布式系统中局部唯一等。

淘宝采用的方案：`时间+不重复值+用户ID后6位`

UUID占36个字节（32个字符+4个`-`），全球唯一、数据无序、插入性能差，故改造：
...
冷热数据分离：把表的全字段按操作频率拆成两组。优点是减少IO，简化表结构，缺点是查询时免不了连接，让事务更加复杂。

建立中间表：把经常连接查询且不经常更新的多表数据抽离成中间表，连接查询就转为对此表的单表查询，更新了就得重新构建。
...
在分库基础上可继续分表，垂直拆分（分属性）或水平拆分（分片-分记录），可继续读写分离。
...
START SLAVE;
# 查看效果，关注Slave_SQL_Running及Slave_IO_Running，确保值都是Yes，一个负责复刻bin.log到relay.log，一个负责语句的执行
SHOW SLAVE STATUS\G;
```
...
- 活动（active）：DML语句正在执行。
- 部分提交（partially committed）：所有DML语句执行完毕，尚未提交。
- 失败（failed）：事务因异常无法继续推进。
- 中止（aborted）：当事务无法推进，不得不撤销事务内之前的DML操作，回滚完毕。
- 提交（committed）：部分提交状态下更新的数据均同步到磁盘中。
...
关于隔离级别，补充脏写-并发执行的两事务修改同一记录。按业务常识，并发问题的严重性排序是：脏写>脏读>不可重复读>幻读。

![隔离级别](mysql.assets/隔离级别.png)

可见可重复读与序列化级别是等效的。

因脏写太过严重，SQL标准规定的及任意DBMS制定的四种隔离级别均解决脏写，即使得写事务并发执行时互斥地更新同一条记录。
...
ACID里的隔离性靠锁机制实现，其他三个靠redo（重做）日志与undo（回滚）日志保证。
...
InnoDB引擎以页为单位管理存储的，所有的DML操作都不是直接操作磁盘上的页，而是先将相关页读入内存中的buffer pool，然后所有的变更操作都是先作用于缓冲池，最后缓冲池里的脏页以一定频率刷入磁盘，故commit很晚的话脏页很可能已经刷入磁盘（考不考虑commit很早而没到刷盘时间点的情况？）。

但当执行commit语句时DBS宕机，脏页都没了，如何保证它们仍刷回到磁盘呢？InnoDB采用WAL（write-ahead logging）技术，刷盘之前先做日志，记录当前事务内的变更操作（逻辑）对应的页操作（物理），那么即使刷盘期间宕机，重启后可从redo日志恢复页操作。做日志比刷盘快得多，降低了做日志时宕机的风险，日志文件占空间也小。
...
上图中的第3步记录页操作受操作系统的优化影响，又分为redo log buffer到page cache、page cache到文件两个过程，那么InnoDB给出如下参数，不恒以一定频率做日志。

```sql
# 以一定频率做3，不关心何时提交，性能次之
SET innodb_flush_log_at_trx_commit = 0;
# 默认，提交一次就做3，性能最差但安全性最佳
...
每当执行增删改语句，都要记录逆逻辑，具体地，当插入一条记录，记录一个DELETE语句；当删除一条记录，记录一个INSERT语句；当修改一条记录，记录一个UPDATE语句。这些记录下来的内容就叫回滚日志。

一旦产生undo日志就随之产生redo日志，因为前者也需要持久性保护。

作用是支撑rollback及MVCC。

文件层面上，InnoDB对undo日志采用段的存储结构，即回滚段（rollback segment），每个回滚段含1024个undo log segment，在每个undo log segment里面申请undo页，undo页存放着前述记录内容。相关参数如下，一般不修改。
...
- uncommitted undo information：已执行但未提交操作的回滚。
- committed undo information：已提交但未过期操作的回滚，故严格意义上已提交操作不一定不可回滚。
- expired undo information：已提交且已过期操作的回滚，此回滚不能实行。
...
DBMS对并发操作进行控制，由此产生锁，实现各隔离级别，影响着并发访问性能。
...
MVCC通过记录的多版本管理实现事务并发控制，既实现了RC、RR级别又提升了并发度、降低死锁概率，如RR级别下， 写事务修改某条记录，持有X锁，读事务想读这条记录不阻塞，因为读到的是undo log支持的旧版本。

MVCC没有正式标准，故不同DBMS的具体实现不尽相同。像MySQL中仅InnoDB引擎实现了MVCC。

辨析快照度（一致性读）和当前读：快照读读到的可能是数据的最新版本，也可能是历史版本，当前读读到的一定是最新版本。在串行化级别下，一定只有当前读，读加锁的记录一定是当前读，因为阻塞的目标就是等到提交完再去读最新版本。

### 核心

我们知道MySQL的可重复读与串行化级别是等效的，靠的就是MVCC，而且不是等价的，即前者一并解决幻读问题的同时并没有损失并发度，也就是不会有锁带来的阻塞情况，在写事务活动期间读事务是可以读的。

聚簇索引所存用户记录带隐藏字段：

- `trx_id`：准备更改当前记录的事务id。
- `roll_pointer`：当前记录被更改后指向在undo log中的更改前版本。每条删改语句（插入语句的undo日志在提交之后被回收）对应的一条undo日志带有一个`roll_pointer`属性，指向一个由历次DML语句执行之后的当前记录连成的链表，其头结点就是最近一次执行之后的版本。

ReadView是某事务进行快照读时产生的读视图。易知RU与串行化级别同ReadView、MVCC不搭界，后者加锁阻塞，前者连未提交的记录都读得到，都是当前读，故MVCC、ReadView只服务于RC、RR级别。

ReadView有如下四项工作：

- 创建或获取这个快照读事务的id，记作`creator_trx_id`。

- 生成处活动状态的事务的id列表，记作`trx_ids`。

- 获取上述列表中值最小的id，记作`up_limit_id`。

- 分配给下一个快照读事务的id，取值为迄今为止所有事务id里的最大值+1，记作`low_limit_id`。

来了个事务T要进行SELECT操作，随即使用ReadView，它就按如下步骤从链表中选出到底去读哪个版本。设当前版本的`trx_id`为x。

1. 若$creator\_trx\_id=x$，则说明此版本就是事务T创造的，当然可以读，退出，否则来到第2步。
2. 若$up\_limit\_id>x$，则说明创造此版本的事务在事务T生成ReadView之前提交，可以读，退出，否则来到第3步。
3. 若$low\_limit\_id \leqslant x$，则说明创造此版本的事务在事物T生成ReadView之后开启，不确定是否提交，故不可读，来到第4步。
4. 若$up\_limit\_id \leqslant x且x<low\_limit\_id$，则说明创造此版本的事务要么活动要么已提交（回滚涉及的版本不存在？），进一步判断x是否在`trx_ids`中，否则可读，是则不可读，来到第5步。
5. 移动到下一个版本。

如何区分RC与RR，很好办，前者使用的ReadView是每SELECT一次新建一个，后者使用的是SELECT时没有才新建，有则复用。
...
即bin log，也叫变更日志（update log），记录了所有已执行的DDL与DML语句等更新事件，像SELECT、SHOW等不属于更新事件，以事件的形式保存在二进制文件中。

有了这个二进制文件，我们可以做如下同步工作：

- 数据恢复：若因意外而丢失数据文件，则可根据bin log保存的事件恢复。
- 数据复制：如主从复制。
...
写入过程：事务每当执行DDL或DML语句，就把日志写到内存里的缓存binlog cache中，事务提交时再从binlog cache转存到binlog文件中，转存的时机受`binlog_cache_size`参数控制，一旦binlog cache容量超过其值便刷盘。类比redo log，这个刷盘也分为write和fsync过程，即先存到page cache，再存到外存以减少磁盘IO，相关参数是`sync_binlog`：

- 取0：每次提交时只write，何时fsync由OS决定。
- 取1：每次提交时write且fsync，性能就差了。
- 取$N(N>1)$：每次提交时仅write，累积N个事务后再fsync。

bin log与redo log均保证持久性，不过前者是逻辑日志，后者是物理日志。有时提交DBS宕机了，redo log有记录但bin log没记录，从库无法同步，对此InnoDB给redo log设置了两阶段提交，保证这两个日志的一致性。

- prepare阶段：执行DML语句之后从buffer pool存页操作记录到redo log cache，完事后附上此阶段标记。
- commit阶段：提交之后进行redo log cache与binlog cache的刷盘，完事后给redo log文件含此阶段标记。

那么提交时DBS宕机，重启后检查redo log文件是否含commit阶段标记，是则从redo log文件恢复页操作并提交，否则说明binlog cache的刷盘之前或之后或期间发生了异常，于是继续判断bin log文件中有无完整相应事件（两日志文件均存多事务id），有则从redo log文件恢复页操作并提交，否则必须回滚，不然主库能恢复，从库怎么办。

bin log文件内容的格式有几种：STATEMENT模式、ROW模式、MIXED模式-视情况选前两个中更好的。

### 中继日志

中继日志（relay log）只在主从DBS架构中的从服务器上，作用是实现主从一致性。
