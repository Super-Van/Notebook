...
由于表达水平有限，理论描述能少则少，请阅读《Head First 设计模式》等书籍，代码这里不列出，请参照项目。
...
请自行参考文章，如[七大基本原则](https://zhuanlan.zhihu.com/p/24614363)。
...
单例模式（Singleton Pattern），就是采取一定方法使得在内存中，某个类只存在一个实例，且该类提供一个取得此实例的方法。

单例模式保证了对于某个类，内存中只有一个对象，一来节约了内存资源，二来避免了频繁创建对象的时间开销。
...
缺点：效率低。当第一个线程已经创建单例，后续线程只有拿单例的可能，其实无需同步。

在实际开发中，不推荐使用此方式。

### 懒汉式-同步代码块

此方式本意是对上一种方法进行改进，因为上一种效率低，但是改了后线程又不安全了，故在实际开发中不能用。

### 懒汉式-双重检查

双重检查（double-check）概念在多线程开发中很常见，既可保证线程安全，又能不牺牲效率。

实例化代码只由首线程执行一次，且后续线程不会再执行同步代码块，直接返回实例。
...
缺点是违反了OCP原则-对扩展开放，对修改关闭。当我们给类增加功能，尽量不要修改现有代码。

注：这里对开闭原则作进一步阐释，项目一旦庞大起来，扩充很方便，但当耦合度高时修改就很不方便，改了一个地方可能其他很多地方都得跟着改，找都不好找。另外，修改体现了程序的不稳定性，改的地方一多就意味着可能存在某种颠覆性的隐患。
...
简单工厂模式是指由一个工厂对象创建产品类的实例（定义工厂类封装实例化产品类的行为），乃是工厂模式家族中最简单实用的。
...
改进之后，对象的创建与使用解耦，提升可维护性。现在要添加新种类披萨的话，只用改工厂的方法这一处代码即可。
...
抽象工厂与工厂方法记忆混淆，还是通过案例来理解。现在想让这两种披萨都能做出武汉和北京这两种风味，要在原料-Pizza类的dough和saurce属性上做文章，我们把原料也视作产品类，于是设计出4种具体产品类，再围绕它们设计出具体创建者类，同时面向接口编程，设计出抽象的产品类与原料工厂，最后让现有的一组抽象产品与创建者-Pizza与PizzaStore依赖这一组抽象。

类图如下：

<img src="Java设计模式.assets/抽象工厂.png" alt="抽象工厂"  />

假定我们将一组相关的产品类与创建者类称作一个家族，那么上图中明显看出两个家族，左边的家族依赖右边的家族。家族就是抽象工厂的特征。

### 总结

将实例化对象的代码提取到一个类中进行统一管理和维护，实现代码复用，提高项目的扩展性和维护性。

获取对象时，不要直接new，而是间接从工厂那里获取，或者说不要直接地将对象赋给变量。
...
缺点：要为每个类配一个clone方法，对于新的类当然可以，但对于已有的类就不友好了，破坏了开闭原则。
...
适配器概念来源于生活，比如各国插头插头插座规格不统一，可以用交流电适配器接通美式插头与欧氏插座，进而让电器更通用。

适配器又叫包装器（Wrapper），援引《Head First 设计模式》对适配器模式（Adapter Pattern）的定义：

>  将一个类的接口转换成客户期望的另一个接口，适配器让原本不兼容的类可以合作无间。

适配器模式主要分三类：类适配器模式、对象适配器模式、接口适配器模式。

工作原理：将类实现的一个接口（被适配者）转换为实现的另一个接口（目标接口或适配者）。用户感觉看不到被适配者，而只和目标接口交互，因为用户调用的是适配器转换得到的目标接口方法，接着适配器再调用被适配者的方法。

### 类适配器

所谓类适配器是指被适配者以类的形式服务于适配器，即让适配器继承被适配者，同时继承（对Java来说只能实现）适配者。
...
- 被适配者的方法暴露给适配器，一来子类的覆盖可能破坏父类的封装，二来父类的修改可能影响到子类，即违反合用复用原则与里氏替换原则。

### 对象适配器

所谓对象适配器是指被适配者以实例的形式服务于适配器，即让适配器不去继承被适配者，而是持有被适配者实例，具体可以是聚合、组合，也可以是参数依赖，这样满足了上述两条原则。
...
当适配细节尚不明确，即暂不知目标接口的哪些方法与被适配者相关，可设计一个抽象适配器来实现（继承）目标接口，对接口全体方法进行空实现，等到细节明确了再安排具体适配器继承（实现）抽象适配器并重写想要重写的方法而不一定是所有方法。

还是电压转换的例子，扩充适配者的输出电压，并灵活新增220V、110V被适配者分别重写输出5V、输出10V的方法：

![接口适配器](java设计模式.assets/接口适配器.png)

但是随着Java 8新特性-接口的默认方法出现，这个接口适配器就无甚意义了。
...
> 桥接模式通过将实现和抽象放在两个不同的类层次中而是它们可以独立改变。

桥接模式（Bridge Pattern）致力于让类的数目尽量少，通过层次内继承与层次间聚合的手段让不同的类层次承担不同的职责。

根据桥接模式的思路，可将手机类图改造为下面这个类图，将多级继承等价为多层聚合，避免了类数量的高速增长。
...
每个单品（具体的主体或附加产品）有各自的单价，求总价就是结合份数对它们进行叠加。

最简单的方案是对这些单品进行任意搭配得到特定的类，每个特定的类都继承同一个主体产品抽象类，问题显而易见，类别增多或某类的单品增多，就造成类数量爆炸，还有代码冗余的问题。

些微改进的方案是像上一章的第一个类图那样将单级继承换替成多级继承，但同样有类数量爆炸问题。

为了缓和类数量爆炸，让附加单品类作主体单品抽象类的实例域。

<img src="java设计模式.assets/装饰者案例.png" alt="装饰者案例" style="zoom:80%;" />

缺点是违反OCP；违反单一职责原则，有的附加单品不适合某些主体单品，从而产生不必要的null处理；违反依赖倒置原则。

为了遵守单一职责原则与依赖倒置原则，继续改进，附加产品抽象类列表作主体产品抽象类的实例域，从而灵活添加附加单品，这就成了上一章的桥接模式。

![装饰者桥接](java设计模式.assets/装饰者桥接.png)

它也有缺点，违反OCP，注意上一章的案例不违反OCP是由于没有主体、附加的语义，本案例特殊之处在于主体产品类家族必须作最抽象的层次即桥接链的最左端，因为只能主体聚合附加，不能主体聚合到附加，既然无法往左边扩展性桥接，那么新增一级类别，就只能让抽象层次再桥接（聚合）一个实现层次，故违反OCP。

最后引出最佳的装饰者模式。

### 改进

援引《Head First 设计模式》对装饰者模式（Decorator Pattern）的定义：

> 装饰者模式动态地将责任附加到对象上。若要扩展功能，装饰者提供了比继承更有弹性的替代方案。

核心是全体附加产品抽象类作装饰者，继承且聚合主体产品抽象类。类图如下：
...
在Java IO中装饰器模式是最亮眼的明星，我们无数次地看见一个流对象传入另一个流的构造器，后者继续传入下一个流的构造器……这些流的超类是一致的，这便体现了一层层的装饰。
...
外观模式（Facade Pattern）过于简单，存在感不强，也容易理解，故代码省略，仅作概述。

一群类形成一个子系统（如dao层），调用部分类的实例来完成某个流程，传统方式就是客户端直接获取一个个对象然后调用它们的方法（如在controller层直接调用dao层）。外观模式则定义一个“充门面”的接口（如service层），利用其下方法将子系统的所有方法分门别类地聚集起来（如service层调用dao层），然后客户端只需使用接口（如controller层调用service层），无需了解内部细节。

特点：

- 对客户端屏蔽了子系统的细节，因而降低了客户端对子系统使用的复杂性。
- 降低了客户端与子系统的耦合，让子系统更容易维护。
- 合理使用外观模式，可帮助我们更好地划分系统的层次。联想三层架构，service层就是外观。
- 不能滥用、任用外观模式，要以改善系统的层次和维护性为目的。
...
即Proxy Pattern，概念：为一个对象提供一个替身，以控制对这个对象的访问，即通过代理对象访问目标对象，好处是在目标对象基础上，增加额外的功能操作。

我们想让目标对象满足OCP原则，很稳定，不修改，但又想随着场景的变化扩充其功能，那么代理对象就应运而生，我们可以通过创建代理对象来扩充功能。
...
可以看出，若让多个目标类实现接口，那么代理类就能给多个目标类扩展相同的功能，也可创建多个代理类给目标类扩展多种功能。

优点：在不改动目标类的前提下，能通过代理对象对其进行功能扩展。

缺点：想让代理类给接口的多个方法扩展相同逻辑，或者想让代理类多实现、给多个接口扩展相同逻辑，这些场景均会造成代码冗余的问题，可维护性也差。由此引出动态代理简化重复工作，让业务逻辑与扩展逻辑解耦。

### 动态代理

代理类无需自定义，也就无需显式实现接口（底层实现），也就免去代理类大量重复写相同逻辑的负担，被代理类要显式实现。

利用反射API里的Proxy类，调用其newProxyInstance方法为动态地为被代理对象生成代理对象，代理对象的结构与此方法的第三个参数-InvocationHandler接口对象的结构相同，但两对象毕竟属不同的类。

类图：

<img src="java设计模式.assets/动态代理.png" alt="动态代理" style="zoom:80%;" />

如图所示，这两个接口的方法的签名大不相同，代理类仍隐式实现接口，将相同操作切到任一接口的任一方法周围。

缺点：写起来麻烦；要求目标类与代理类必须实现同一接口，最好能为未实现任何接口的类对象创建代理对象以及直接为接口创建代理对象，由此引出Cglib代理。

### Cglib代理

静态代理和动态代理都要求目标对象实现同一个接口，但有时目标类并没有实现接口或者说我们希望直接代理，这时可通过目标类的子类对象完成代理，即Cglib代理。

Cglib代理又叫子类代理，是在内存中动态为目标对象构建一个子类对象，让它重写方法，以实现功能扩展。在底层它通过字节码处理框架ASM来转换字节码而生成新的类（代理类），运行新字节码创建的对象就是运行代理对象了。

Cglib是一个高性能代码生成包，广泛用于AOP框架。
...
虽说它俩没实现同一接口，但代理类还是要实现的一个重要的方法拦截器。
...
为什么不用继承，语法上确实可惜，但语义上不合理，第三方平台毕竟不是气象站，不能自己测量得到数据。

缺点：面向具体实现而不是面向接口编程，违反了OCP，即增加第三方平台类都会影响Weather类，不利于维护。
...
优点：以集合的方式管理观察者，能够方便地注册、移除和通知；注册新的第三方平台类不会违背OCP。
