...
this指向事件源，即被注册事件的元素亦即回调函数的调用者，而`e.target`指向触发事件的元素，即冒泡机制传播链上的起始元素。`e.currentTarget`指向事件源。示例如下：
...
this指向的当前方法的调用者。如window对象、document对象、dom元素、普通对象。

this的指向是可变的，我们后面再谈到。

#### js执行机制

js语言的典型特征是单线程，为谋求高效率，同时执行多个任务，H5利用多核CPU的计算能力，提出Web Worker标准，于是在js中出现了同步和异步两种机制：

- 同步：某一个任务执行完之后才能执行其后一个任务。
- 异步：某一个任务执行期间就能执行其后一个任务。
...
异步任务结束时其回调函数被添加到任务队列（也叫消息队列）中。

JS的执行机制是：

1. 先执行执行栈中的同步任务。
2. 一旦某个回调函数绑定的事件发生（如定时器到期），就将此回调函数放入消息队列，但不执行。
3. 执行栈里的所有同步任务执行完毕，系统就按序读取任务队列里的异步任务，将其放入执行栈执行。

从主线程执行栈到任务队列之间来回运作的过程叫事件循环（event loop）。
...
正则表达式（regular expression）是用于匹配字符序列的样板。在JS中，它以对象的形式存在。

具体而言，正则表达式用来检索或替换满足某种规则的文本。应用如表单验证、过滤敏感词等。
...
应着重理解匹配一词的意思，即只要待检验字符串中存在满足指定规则的字符序列，就得到true的结果。
...
JavaScript Object Notation-JS对象标记，是一种标准的轻量级的数据交换格式（90%以上的系统交换数据用json）。特点是体积小、易解析。实际开发中用的最多的两种数据交换格式是JSON和XML，后者虽然体积大、解析麻烦，但更严谨。
...
this的指向并不是一成不变的，但永远指向方法的调用者，有时调用者是实例对象，有时是某个事件源。有例为证：
