...
- DML：数据操纵语言，包括insert、delete、update、select。民间将select单独分出来叫DQL，Oracle没这么做。
...
- atomicity-原子性：指事务是一个不可分割的工作单位，即诸DML操作要么都成功要么都不做，不能仅部分成功。
- consistency-一致性：指事物让数据库从一个一致性状态变为另一个一致性状态，这个一致性指业务上（或语义上）的一致性。如对转账业务，某记录的余额分量减小，则必有另一记录的余额分量增大。
- isolation-隔离性：广义上指一个事务的执行不能被其他事务干扰，分情况讨论，有读事务与读事务的并发、读事务与写事务的并发、写事务与写事务的并发，尤其要求写事务并发执行时互斥地更新同一条记录；狭义上特指在并发执行过程中写事务对读事务的影响，具体指四种隔离级别。
- durability-持久性：指一旦事务被提交，对数据库的改变就是永久性的，不会受宕机等影响。

原子性是重中之重。

事务生命周期：如下图所示，开始于第一条增删改语句，终结于提交或回滚。

<img src="oracle.assets/事务生命周期.png" alt="事务生命周期" style="zoom:80%;" />

交叉阅读mysql笔记，理解提交和回滚的细节。
...
理解回滚（回退）的机制，回滚就是撤销，只针对已执行而未提交的DML语句，实现方式有两种：

- 显式回滚：输入<span id="rollback">rollback命令</span>，默认回滚到当前事务开始之前。
- 隐式回滚：宕机、断电、异常断开连接等异常情况。
...
- 不可重复读：基于同一条查询语句，B在A提交DML语句前后读到不一样的记录或记录缺失。
- 幻读（虚读）：基于同一条查询语句，B在A提交DML语句前后读到更多的记录。
...
序列的作用是手动实现列值自增，本质是内存中的数组，数组长度固定。
...
裂缝问题：比方说数组长度为20，范围就是1到20，当前拿到6，但下一次拿到的是21。列举一些情况：
