...
- consistency：一致性，指事物让数据库从一个一致性状态变为另一个一致性状态，这个一致性指业务上（或语义上，与语法无关）的一致性。如对转账业务，某记录的余额分量减小，则必须使另一记录的余额分量增大。
- isolation：隔离性。广义上指一个事务的执行不能被其他事务干扰，分情况讨论，有读事务与读事务的并发、读事务与写事务的并发、写事务与写事务的并发，尤其要求写事务并发执行时互斥地更新同一条记录；狭义上特指在并发执行过程中写事务对读事务的影响，由此产生四种隔离级别。
...
- 不可重复读：基于同一条查询语句，B在A提交DML语句前后读到不一样的记录或记录缺损。
...
由上例可知，系统的数据校验应当在前端、后端、数据库多处进行，当然越早拦截越好。
